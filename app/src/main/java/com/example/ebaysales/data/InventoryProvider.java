...
public class InventoryProvider extends ContentProvider {
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        ...
        if (nameProduct == null) {
            throw new IllegalArgumentException("Item name requires");
        }

        Integer priceProduct = values.getAsInteger(InventoryEntry.COLUMN_PRODUCT_PRICE);
        if (priceProduct != null && priceProduct < 0) {
            throw new IllegalArgumentException("Item price requires valid");
        }
        ...
        if (quantityProduct != null && quantityProduct < 0) {
            throw new IllegalArgumentException("Item quantity requires valid");
        }

        Integer conditionName = values.getAsInteger(InventoryEntry.COLUMN_PRODUCT_CONDITION);
        if (conditionName == null || !InventoryEntry.isValidConditionName(conditionName)) {
    ...
    private int updateProduct(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(InventoryEntry.COLUMN_PRODUCT_PRICE)) {
            ...
            if (priceProduct != null && priceProduct < 0) {
                throw new
                        IllegalArgumentException("Item price requires valid");
            }
        }
        ...
        if (values.containsKey(InventoryEntry.COLUMN_PRODUCT_QUANTITY)) {
            ...
            if (quantityProduct != null && quantityProduct < 0) {
                throw new
                        IllegalArgumentException("Item quantity requires valid");
            }
        }
        if (values.containsKey(InventoryEntry.COLUMN_PRODUCT_CONDITION)) {
            Integer conditionName = values.getAsInteger(InventoryEntry.COLUMN_PRODUCT_CONDITION);
            if (conditionName == null || !InventoryEntry.isValidConditionName(conditionName)) {
                throw new IllegalArgumentException("Condition requires valid");
            }
        }
